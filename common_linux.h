...
#define SYSCTL_CONF_FILE "/etc/sysctl.conf"
#define MOUNT_DIR "/mnt/huge"
#define NUM_PAGES 100
#define PAGE_SIZE_2MB 1024 * 1024 * 2
#define PAGE_SIZE_1GB 1024 * 1024 * 1024

static long syz_mount_huge(void)
{
	char command[100];
	int res;

	snprintf(command, sizeof(command), "mount -t hugetlbfs nodev /mnt/huge");
	res = system(command);
	snprintf(command, sizeof(command), "mkdir /mnt/huge");
	res = system(command);

	snprintf(command, sizeof(command), "touch /mnt/huge/a");
	res = system(command);

	snprintf(command, sizeof(command), "touch /mnt/huge/b");
	res = system(command);

	snprintf(command, sizeof(command), "touch /mnt/huge/c");
	res = system(command);

	snprintf(command, sizeof(command), "chmod a+w /mnt/huge/c");
	res = system(command);

	snprintf(command, sizeof(command), "echo 'TEST_STRING' > /mnt/huge/c");
	res = system(command);

	int fd;

	fd = open("/mnt/huge/c", O_RDWR | O_CREAT | O_TRUNC, 0777);
	if (fd == -1) {
		debug("Error opening c file\n");
	}
	...
	if (bytes_written == -1) {
		debug("Error writing to file c\n");
	}
	close(fd);

	snprintf(command, sizeof(command), "cat /mnt/huge/c");
	res = system(command);
	/* snprintf(command, sizeof(command), "ls /mnt/huge > tmp.txt"); */
	/* res = system(command); */
	/**/
	/* snprintf(command, sizeof(command), "cat tmp.txt"); */
	/* res = system(command); */

	debug("here we are... common_linux.h line 4090\n");
...
static long syz_hugetlb(volatile long num)
{
	char command[100];
	int res;
	// num = 20;
	snprintf(command, sizeof(command), "%s %ld %s", "echo ", num, " > /proc/sys/vm/nr_hugepages");
	res = system(command);
	return 0;
}
static long syz_hugetlb_surplus(volatile long num)
{
	char command[100];
	int res;
	snprintf(command, sizeof(command), "%s %ld %s", "echo ", num, " > /proc/sys/vm/nr_overcommit_hugepages");
	res = system(command);
	return 0;
}

static long syz_disable_hugepages(void)
{
	char command[100];
	int res;
	snprintf(command, sizeof(command), "echo %d /proc/sys/vm/nr_hugepages", 0);
	res = system(command);
	return 0;
}

static long syz_disable_transparent(void)
{
	char command[100];
	int res;
	snprintf(command, sizeof(command), "echo never > /sys/kernel/mm/transparent_hugepage/enabled");
	res = system(command);
	return 0;
}

static long syz_enable_transparent(void)
{
	char command[100];
	int res;
	snprintf(command, sizeof(command), "echo always > /sys/kernel/mm/transparent_hugepage/enabled");
	res = system(command);
	return 0;
}
...
static long syz_read_from_page(volatile long addr)
{
	debug("Read from huge page: %s\n", (char*)addr);
	return 0;
}

static long syz_create_hugepage_fd(volatile long num)
{
	char filename[256];
	snprintf(filename, sizeof(filename), HUGEPAGE_DIR "/hugepagefile%ld", num);
	int fd = open(filename, O_CREAT | O_RDWR, 0755);
	if (fd == -1) {
		debug("Failed to create huge page file");
	}

	if (ftruncate(fd, HUGEPAGE_SIZE) == -1) {
		debug("Failed to truncate huge page file");
	}

	return (volatile long)fd;
}

static long syz_write_to_hugepage(volatile long map)
{
	char msg[256];
	snprintf(msg, sizeof(msg), "Huge Page %s", (char*)map);
	strcpy((char*)map, msg);
	debug("Written to huge page: %s\n", (char*)map);
	return 0;
}

static long syz_delete_hugepage(volatile long fd)
{
	if (close((int)fd) == -1) {
		debug("Failed to close huge page file");
	}
	char filename[256];
	snprintf(filename, sizeof(filename), HUGEPAGE_DIR "/hugepagefile%d", 0);
	if (remove(filename) == -1) {
		debug("Failed to remove huge page file");
	}
	return 0;
}
#endif
